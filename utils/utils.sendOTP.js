...
export const sendOTP = async (User_Email, User_Phone) => {
  try {
    ...
    const message = `Thank you for using Mex Printing. ${User_OTP_Code} is your one time password (OTP). 
        Kindly note that your OTP expires in 60 minutes`;
    // set the unique key
    cache.put(
      User_Phone,
      User_OTP_Code,
      parseInt(secrets.MINUTES_MEMCACHE) * 60 * 1000,
      function (key, value) {
    ...
    // });
    await sendEmail(User_Email, "MEX Printing - OTP", message);
